...
ccall(H,P,G):-vars_of(G,Vs),ccall(H,P,Vs,G,the(Vs)).  
  
ccall(G):-ccall(localhost,10001,G).

update_cserver(S,G):-vars_of(G,Vs),update_cserver(S,Vs,G,the(Vs)).
...
gg1:-
 G=member(X,[m,n,p,q]),
 open_csocket(S),
 ( celement_of(S,G,X),println(X),fail
 ; close_csocket(S)
 ).
 
gg:-
  open_csocket(localhost,10001,S),
    load_cserver(S,X,member(X,[10,20,30])),
      ask_cserver(S,I),
      ask_cserver(S,J),
      thank_cserver(S),
      println([I,J]),  
    load_cserver(S,X,member(X,[aaa,bbb,ccc])),
      ask_cserver(S,A),
      ask_cserver(S,B),
      ask_cserver(S,C),
      ask_cserver(S,D),
