...
bg(G):-
  ( get_verbosity(V),V>2->
    traceln('bg/1 deprecated, symgc disabled, use bg(Goal,Hub) and stop(Hub) to control thread')
  ; true
  ),
  bg(G,_H).

bg_clone(G):-
  traceln('bg_clone/2 deprecated, use bg_clone(Goal,Hub) and stop(Hub) to control thread'),
  bg_clone(G,_Hub).
...
mt_test2:-set_verbosity(10,_),for(_,1,100),mt_test2(5000,_),fail.
