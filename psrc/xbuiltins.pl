...
default_ops(Ops):-new_ops(0,Ops).

get_pri(Op,Assoc, Pri):-default_ops(Ops),get_pri(Ops,Op,Assoc, Pri).
get_pris(Op,Pris):-default_ops(Ops),get_pris(Ops,Op,Pris).
op_succ(Pri,NewPri):-default_ops(Ops),op_succ(Ops,Pri,NewPri).
op_pred(Pri,NewPri):-default_ops(Ops),op_pred(Ops,Pri,NewPri).
all_ops(OpList):-default_ops(Ops),all_ops(Ops,OpList).
op_info(Op,Assoc,Pri):-default_ops(Ops),op_info(Ops,Op,Assoc,Pri).
is_op(Op):-default_ops(Ops),is_op(Ops,Op).

%% current_op(Pri,Assoc,Op): lists/checks what current operators are in effect
current_op(Pri,Assoc,Op):-
  default_ops(Ops),
  current_op(Ops,Pri,Assoc,Op).

current_op(Ops,Pri,Assoc,Op):-nonvar(Op),!,current_op1(Ops,Pri,Assoc,Op). 
current_op(Ops,Pri,Assoc,Op):-
  all_ops(Ops,OpList),
  member(Op,OpList),
  op_info(Ops,Op, Assoc,Pri).

current_op1(Ops,Pri,Assoc,Op):-nonvar(Assoc),!,get_pri(Ops,Op,Assoc, Pri).  
current_op1(Ops,Pri,Assoc,Op):-op_info(Ops,Op, Assoc,Pri).

op_info(Ops,Op, Assoc,Pri):-get_pris(Ops,Op,Pris),member(op(Assoc,Pri),Pris).

is_op(Ops,Op):-get_pris(Ops,Op,Pris),Pris=[_|_].
...
%% reset_global_gensym(Base): resets gensym asocciated to base - global to process
...
%% ground_set_difference(As,Bs,Cs): computes the set difference without duplicates of ground As,Bs
...
%% ground_set_union(As,Bs,Cs): computes the set union without duplicates of ground As,Bs
...
%% ground_set_union(As,Bs,Cs): computes the set intersection without duplicates of ground As,Bs    
