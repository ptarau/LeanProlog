...
%% vars_of(Term,Vars) :: extracts set of free varibles Vars occurring in Term
vars_of(Term,Vars):-free_variables(Term,[],[],Vars).

%% term_variables(Term,Vars) :: extracts set of free variables occurring in Term
% term_variables(Term,Vars):-free_variables(Term,[],[],Vars). % Paulo

term_variables(Term, Vars) :-
  '$term_to_variables'(Term, [], Vars0),
  reverse(Vars0, Vars).


'$term_to_variables'(Term, Vars0, Vars) :-
  var(Term),
  !,
  ( '$var_member_chk'(Term, Vars0) ->
    Vars = Vars0
  ; Vars = [Term| Vars0]
  ).
...
'$term_to_variables'(Term, Vars0, Vars) :-
  Term =.. [_| Args],
  '$term_to_variables_list'(Args, Vars0, Vars).

'$term_to_variables_list'([], Vars, Vars).
'$term_to_variables_list'([Term| Terms], Vars0, Vars) :-
  '$term_to_variables'(Term, Vars0, Vars1),
  '$term_to_variables_list'(Terms, Vars1, Vars).


'$var_member_chk'(Var, [Head| Rest]) :-
  ( Var == Head ->
    true
  ; '$var_member_chk'(Var, Rest)
  ).
