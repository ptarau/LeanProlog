...
%maybe_ensure_canonical(X,R):-is_prolog(lprolog),!,X=R.
maybe_ensure_canonical(X,R):-ensure_canonical(X,R).

ensure_canonical(X,R):-var(X),!,R=X.
ensure_canonical(X,R):-atom(X),!,R=X.
%ensure_canonical(X,R):-integer(X),!,R=X.
%ensure_canonical(X,R):-integer(X),!,to_string(X,C),R='$i'(C).
%ensure_canonical(X,R):-float(X),!,to_string(X,C),functor(R,'$number',1),arg(1,R,C).
...
*/
mdef_to_def((H:-B),(H:-NewB)):-functor(H,F,N),maybe_crossref_def(F/N),replace_body(B,NewB,F/N).
...
replace_body(T0,T,FN):-maybe_ensure_canonical(T0,T),maybe_crossref_collect(FN,T).  

maybe_crossref_def(_):- \+(is_prolog(lprolog)),!.
maybe_crossref_def(FN):-crossref_def(FN).

maybe_crossref_collect(_,_):- \+(is_prolog(lprolog)),!.
maybe_crossref_collect(_,T):- is_compiled(T),!.
maybe_crossref_collect(_,T):- is_dynamic(T),!.
maybe_crossref_collect(FN,T):-crossref_collect(FN,T).
%maybe_crossref_collect(_,_).
...
split_is(X,A)-->{is_prolog(lprolog),ground(A)},!,{X is A}. % compile time exec - only for lprolog - it has bignums !
