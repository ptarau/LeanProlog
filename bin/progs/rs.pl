...
mgo(Threads):-
    statistics(runtime,[Start|_]),
    multi_sudoku(Threads,s9x9a,Solution),
    write(solved),nl,
    display_board(Solution),
    statistics(runtime,[End|_]),
    T is End-Start,
...
multi_sudoku(Threads,Problem,Xss):-
    call(Problem,Xsss),Xsss=[Xss,_,_],
    %display_board(Xss),
    preprocess([1,2,3,4,5,6,7,8,9],Xsss,XYss),
    ran_problem_goal(Threads,XYss,XGs),
    multi_first(XGs,XYss).
...
ran_problem_goal(Threads,Ps,XGs):-
  XG=(Ps:-ranpermute(Ps,Qs),display_board(Qs),solve(Qs)),
...
display_board(Xss):-numbervars(Xss,1,_),map1(writeOne,Xss),fail;nl.

writeOne(Xs):-traceln(Xs).
