...
pbm:-  
  ...
  findall(X,uav(X),Xs),
  psum(Xs,NT),
  write('NORMALIZED_AVERAGE_TIME, the shorter the better'(NT)),nl,
  abolish(uav/1),
  abolish(ua/3),
  abolish(dix/2).
...
psum([],0).
psum([X|Xs],R):-psum(Xs,R0),R is R0+X.

bm1:-do_bench_mark(assert_unit).

bm:-for(_,1,20),bm2,fail;true.

%bm2:-do_bench_mark(setof),fail.
%bm2:-do_bench_mark(pair_setof),fail.
%bm2:-do_bench_mark(double_setof),fail.
bm2:-do_bench_mark(bagof),fail.
bm2:-write(done),nl.
...
% 13
bench_mark(trail_variables, 20000, trail, dummy).

% 14
bench_mark(medium_unify, 12000, equal(Term1, Term2), dummy(Term1, Term2)) :-
   term64(Term1),
   term64(Term2).
bench_mark(deep_unify, 30, equal(Term1, Term2), dummy(Term1, Term2)) :-
   term4096(Term1),
   term4096(Term2).
...
% 18
bench_mark(index, 3000, ix(1), dummy(1)).

% 19
bench_mark(assert_unit, 1000, assert_clauses(L), dummy(L)) :-
   abolish(ua/3),
   create_units(1, 5, L).
...
bench_mark(double_setof, 1000, setof((X,S), setof(Y, pr(X, Y), S), T),dummy(S, setof(Y, pr(X, Y), S), T)).
bench_mark(bagof, 1000, bagof(X, (Y^pr(X, Y)), S), dummy(X, (Y^pr(X, Y)), S)).


% 23a
bench_mark(functor_in(N), 500, functor_in(T), dummy(T)) :-
   functors(N),
   functor(T, f, N).

% 23b
bench_mark(functor_out(N), 500, functor_out(f, N), dummy(f, N)) :-
   functors(N).

% 23c
bench_mark(functor_unify(N), 500, functor_unify(T, f, N), dummy(T, f, N)) :-
   functors(N),
   functor(T, f, N).

% 24
bench_mark(call_test(G), 200000, call_test(G), call_dummy(G)) :-
   call_goal(G).
/*
% : 25
...
get_cpu_time(T) :- statistics(runtime, [T|_]). %,T is T0/10.
